 ...
 */
public class QualitymetamodelOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "QualityMetamodel.resource.qualitymetamodel.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "QualityMetamodel.resource.qualitymetamodel.ui.occurences.declaration";
	...
	private QualityMetamodel.resource.qualitymetamodel.ui.QualitymetamodelTokenScanner tokenScanner;
	...
	private QualityMetamodel.resource.qualitymetamodel.IQualitymetamodelTextResource textResource;
	 ...
	 */
	public QualitymetamodelOccurrence(QualityMetamodel.resource.qualitymetamodel.IQualitymetamodelTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, QualityMetamodel.resource.qualitymetamodel.ui.QualitymetamodelTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new QualityMetamodel.resource.qualitymetamodel.mopp.QualitymetamodelMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(QualityMetamodel.resource.qualitymetamodel.ui.QualitymetamodelBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, QualityMetamodel.resource.qualitymetamodel.ui.QualitymetamodelPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, QualityMetamodel.resource.qualitymetamodel.ui.QualitymetamodelPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, QualityMetamodel.resource.qualitymetamodel.ui.QualitymetamodelPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == QualityMetamodel.resource.qualitymetamodel.ui.QualitymetamodelPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(QualityMetamodel.resource.qualitymetamodel.ui.QualitymetamodelOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(QualityMetamodel.resource.qualitymetamodel.ui.QualitymetamodelOccurrence.DECLARATION_ANNOTATION_ID);
	}
