...
public class QualitymetamodelDefaultNameProvider implements QualityMetamodel.resource.qualitymetamodel.IQualitymetamodelNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
