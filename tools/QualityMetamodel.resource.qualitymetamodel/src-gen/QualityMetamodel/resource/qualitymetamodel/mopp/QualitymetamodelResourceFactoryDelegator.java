...
public class QualitymetamodelResourceFactoryDelegator implements org.eclipse.emf.ecore.resource.Resource.Factory {
	
	protected java.util.Map<String, org.eclipse.emf.ecore.resource.Resource.Factory> factories = null;
	
	public QualitymetamodelResourceFactoryDelegator() {
		init();
	}
	...
	protected void init() {
		...
		if (new QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelRuntimeUtil().isEclipsePlatformAvailable()) {
			new QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelEclipseProxy().getResourceFactoryExtensions(factories);
		}
		if (factories.get("") == null) {
			factories.put("", new QualityMetamodel.resource.qualitymetamodel.mopp.QualitymetamodelResourceFactory());
		}
	}
