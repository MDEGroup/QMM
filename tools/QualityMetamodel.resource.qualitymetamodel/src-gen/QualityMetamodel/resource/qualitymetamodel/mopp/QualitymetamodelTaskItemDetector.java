...
/**
 * The QualitymetamodelTaskItemDetector is used to find task items in text
 * documents. The current implementation searches for specific keywords to detect
 * task items. The QualitymetamodelTaskItemDetector is used both by the
 * TaskItemBuilder and the editor.
 */
public class QualitymetamodelTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<QualityMetamodel.resource.qualitymetamodel.mopp.QualitymetamodelTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<QualityMetamodel.resource.qualitymetamodel.mopp.QualitymetamodelTaskItem> foundItems = new java.util.ArrayList<QualityMetamodel.resource.qualitymetamodel.mopp.QualitymetamodelTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new QualityMetamodel.resource.qualitymetamodel.mopp.QualitymetamodelTaskItem(keyword, message, localLine, offset, end));
