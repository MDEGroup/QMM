 ...
 */
public class QualitymetamodelMinimalModelHelper {
	
	private final static QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelEClassUtil eClassUtil = new QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelListUtil.castListUnchecked(value);
