 ...
 */
public class QualitymetamodelDebuggableInterpreter<ResultType, ContextType> extends QualityMetamodel.resource.qualitymetamodel.debug.AbstractQualitymetamodelDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private QualityMetamodel.resource.qualitymetamodel.util.AbstractQualitymetamodelInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private QualityMetamodel.resource.qualitymetamodel.IQualitymetamodelCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public QualitymetamodelDebuggableInterpreter(QualityMetamodel.resource.qualitymetamodel.util.AbstractQualitymetamodelInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new QualityMetamodel.resource.qualitymetamodel.IQualitymetamodelInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public QualityMetamodel.resource.qualitymetamodel.util.AbstractQualitymetamodelInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		QualityMetamodel.resource.qualitymetamodel.IQualitymetamodelLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private QualityMetamodel.resource.qualitymetamodel.IQualitymetamodelLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		QualityMetamodel.resource.qualitymetamodel.IQualitymetamodelCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(QualityMetamodel.resource.qualitymetamodel.debug.EQualitymetamodelDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelEObjectUtil.getDepth(current);
		stopCondition = new QualityMetamodel.resource.qualitymetamodel.IQualitymetamodelCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelEObjectUtil.getDepth(current) - 1;
		stopCondition = new QualityMetamodel.resource.qualitymetamodel.IQualitymetamodelCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
