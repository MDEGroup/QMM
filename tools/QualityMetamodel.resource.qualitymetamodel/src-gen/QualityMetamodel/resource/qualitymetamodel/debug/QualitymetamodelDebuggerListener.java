 ...
 * code).
 */
public class QualitymetamodelDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private QualityMetamodel.resource.qualitymetamodel.debug.AbstractQualitymetamodelDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelPair<String, Object>>();
	...
	public QualitymetamodelDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		QualityMetamodel.resource.qualitymetamodel.debug.QualitymetamodelDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(QualityMetamodel.resource.qualitymetamodel.debug.EQualitymetamodelDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(QualityMetamodel.resource.qualitymetamodel.debug.EQualitymetamodelDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(QualityMetamodel.resource.qualitymetamodel.debug.EQualitymetamodelDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(QualityMetamodel.resource.qualitymetamodel.debug.EQualitymetamodelDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(QualityMetamodel.resource.qualitymetamodel.debug.EQualitymetamodelDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(QualityMetamodel.resource.qualitymetamodel.debug.EQualitymetamodelDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(QualityMetamodel.resource.qualitymetamodel.debug.EQualitymetamodelDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(QualityMetamodel.resource.qualitymetamodel.debug.EQualitymetamodelDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelStringUtil.encode('#', stack);
				QualityMetamodel.resource.qualitymetamodel.debug.QualitymetamodelDebugMessage message = new QualityMetamodel.resource.qualitymetamodel.debug.QualitymetamodelDebugMessage(QualityMetamodel.resource.qualitymetamodel.debug.EQualitymetamodelDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(QualityMetamodel.resource.qualitymetamodel.debug.EQualitymetamodelDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				QualityMetamodel.resource.qualitymetamodel.debug.QualitymetamodelDebugMessage message = new QualityMetamodel.resource.qualitymetamodel.debug.QualitymetamodelDebugMessage(QualityMetamodel.resource.qualitymetamodel.debug.EQualitymetamodelDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(QualityMetamodel.resource.qualitymetamodel.debug.EQualitymetamodelDebugMessageTypes.GET_VARIABLES)) {
				...
				QualityMetamodel.resource.qualitymetamodel.debug.QualitymetamodelDebugMessage message = new QualityMetamodel.resource.qualitymetamodel.debug.QualitymetamodelDebugMessage(QualityMetamodel.resource.qualitymetamodel.debug.EQualitymetamodelDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelPair<String, Object> pair = new QualityMetamodel.resource.qualitymetamodel.util.QualitymetamodelPair<String, Object>(name, value);
	...
	public QualityMetamodel.resource.qualitymetamodel.debug.AbstractQualitymetamodelDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(QualityMetamodel.resource.qualitymetamodel.debug.AbstractQualitymetamodelDebuggable debuggable) {
